...
def f2():
    with open("./results/c1_evictDUMBS_w80r80.json", "r") as f:
        res_DUMB_SIMPLE = dict(json.load(f))
    with open("./results/c1_evictDUMBR_w80r80.json", "r") as f:
        res_DUMB_RANDOM = dict(json.load(f))
    with open("./results/c1_evcitLRU_w80r80.json", "r") as f:
        res_LRU = dict(json.load(f))
        res_LRU_sample = {key: res_LRU[key] for key in res_DUMB_RANDOM}
    with open("./results/c1_RDMA_w0r20.json", "r") as f:
        res_RDMA = dict(json.load(f))
